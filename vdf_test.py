...
def check(name: str, condition: bool, failures: list[str], extra: str = ""):
...
def main():
    ...
    check("mod_sqrt round-trip", pow(y_sqrt, 2, p_sloth) == x_qr, failures)
    check("quadratic residue detects square", is_quadratic_residue(x_qr, p_sloth), failures)
    check("fp2_mul returns a pair", len(fp2_mul((3, 5), (7, 11), p_sloth)) == 2, failures)
    ...
    check("fp2_sqrt round-trip on perfect square", fp2_mul(sqrt_elem, sqrt_elem, p_sloth) == elem_square, failures)

    N_test, _ = generate_vdf_setup(512)
    g_derived = hash_to_group(b"test_entropy", N_test)
    check("hash_to_group in range", 2 <= g_derived < N_test, failures)
    check("hash_to_group deterministic", g_derived == hash_to_group(b"test_entropy", N_test), failures)
    ...
    check("Wesolowski verify passes", wesolowski_verify_vdf(N_w, g_w, y_w, t_w, pi_w), failures)
    check("Wesolowski rejects tampered y", not wesolowski_verify_vdf(N_w, g_w, (y_w + 1) % N_w, t_w, pi_w), failures)
    ...
    check("Pietrzak verify passes for power-of-two t", pietrzak_verify_vdf(N_p, g_p, y_p, t_p, pi_p), failures)
    check("Pietrzak proof depth is logarithmic", len(pi_p) <= t_p.bit_length(), failures)

    try:
        pietrzak_eval_vdf(N_p, g_p, 3)
    except ValueError:
        check("Pietrzak rejects non-power-of-two t", True, failures)
    else:
        check("Pietrzak rejects non-power-of-two t", False, failures)
    ...
    check("Hash chain verify passes", hash_chain_verify(x_hc, y_hc, t_hc, ckpts), failures)
    check("Hash chain rejects wrong y", not hash_chain_verify(x_hc, y_hc[:-2] + "ff", t_hc, ckpts), failures)
    ...
    check("Sloth verify matches deterministic eval", sloth_ok, failures)

    x_s = secrets.randbelow(p_s - 1) + 1
    y_s = sloth_eval(x_s, p_s, 4)
    check("Sloth rejects wrong y", not sloth_verify(x_s, (y_s + 1) % p_s, p_s, 4), failures)
    ...
    check("Sloth++ eval/verify stay aligned", sloth_pp_ok, failures)

    x_pp = (secrets.randbelow(p_pp - 1) + 1, secrets.randbelow(p_pp - 1) + 1)
    y_pp = sloth_plus_plus_eval(x_pp, p_pp, 4)
    y_pp_bad = ((y_pp[0] + 1) % p_pp, y_pp[1])
    check("Sloth++ rejects wrong y", not sloth_plus_plus_verify(x_pp, y_pp_bad, p_pp, 4), failures)
    ...
    check("Rational map verify passes", rational_map_verify(x_rm, y_rm, p_rm, s_rm, a_rm), failures)
    check("Rational map rejects wrong y", not rational_map_verify(x_rm, (y_rm + 1) % p_rm, p_rm, s_rm, a_rm), failures)
    ...
    check("Large prime product verify passes", large_prime_product_verify(N_lp, g_lp, y_lp, t_lp), failures)
    check("Large prime product rejects tampered output", not large_prime_product_verify(N_lp, g_lp, (y_lp + 1) % N_lp, t_lp), failures)
    ...
    check("Beacon Wesolowski verify passes", verify_randomness_beacon(beacon_w), failures)

    beacon_p = randomness_beacon(entropy, N_b, g_b, 256, scheme="pietrzak")
    check("Beacon Pietrzak verify passes", verify_randomness_beacon(beacon_p), failures)

    tampered = dict(beacon_w, beacon_hex="00" * 32)
    check("Beacon rejects tampered output", not verify_randomness_beacon(tampered), failures)
    ...
    enc1_resp = client.post("/api/encrypt", json={"exam_text": "exam A", "t_squarings": 4, "bits": 256, "scheme": "wesolowski"})
    enc2_resp = client.post("/api/encrypt", json={"exam_text": "exam B", "t_squarings": 4, "bits": 256, "scheme": "wesolowski"})
    enc1 = enc1_resp.get_json()
    enc2 = enc2_resp.get_json()

    check("Backend encrypt succeeds for exam 1", enc1_resp.status_code == 200 and "exam_id" in enc1, failures)
    check("Backend encrypt succeeds for exam 2", enc2_resp.status_code == 200 and "exam_id" in enc2, failures)
    if "exam_id" in enc1 and "exam_id" in enc2:
        check("Backend returns distinct exam ids", enc1["exam_id"] != enc2["exam_id"], failures)
        check("Backend returns distinct access tokens", enc1["access_token"] != enc2["access_token"], failures)

        bad_progress = client.get(
            "/api/progress",
            query_string={"exam_id": enc1["exam_id"], "access_token": enc2["access_token"]},
        )
        check("Progress rejects wrong token", bad_progress.status_code == 403, failures)
        ...
        check("Audit is namespaced per exam", audit1["exam_id"] == enc1["exam_id"] and audit2["exam_id"] == enc2["exam_id"], failures)

        solve1 = client.post(
            "/api/solve",
            json={"exam_id": enc1["exam_id"], "access_token": enc1["access_token"]},
        )
        check("Solve starts for exam 1", solve1.status_code == 200, failures)
        status_code_1, progress1 = poll_exam_progress(client, enc1["exam_id"], enc1["access_token"])
        check("Exam 1 solve completes", status_code_1 == 200 and progress1["status"] == "done", failures)
        check("Exam 1 decrypts its own plaintext", progress1["decrypted_text"] == "exam A", failures)

        idle_progress = client.get(
            "/api/progress",
            query_string={"exam_id": enc2["exam_id"], "access_token": enc2["access_token"]},
        )
        check("Exam 2 remains idle until solved", idle_progress.status_code == 200 and idle_progress.get_json()["status"] == "idle", failures)
        ...
        check("Proof generation starts for solved exam", proof_start.status_code == 200, failures)
        _, proof_done = poll_proof_progress(client, enc1["exam_id"], enc1["access_token"])
        check("Proof generation completes", proof_done["proof_status"] == "done" and len(proof_done["vdf_proof"]) > 0, failures)
        ...
        check("Generated proof verifies publicly", verify_resp.status_code == 200 and verify_data["is_valid"], failures)
